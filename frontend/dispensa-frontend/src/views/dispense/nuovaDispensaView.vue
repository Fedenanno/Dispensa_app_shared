...
export default {
    ...
    methods: {
        async createDispensa() {

            await axios.post("dispense/", {
                nome_dispensa: this.name,
                condivisa: this.showInfoShared,
            }) //useAuthStore().register(this.name, this.email, this.password);
            router.push('/');
        },
    },
    watch: {
        showModalInfo: function (val) {
            if (val) {
                this.modalInfo.show();
            } else {
                this.modalInfo.hide();
    ...
    mounted() {
        // infoModal
        const $modalElementInfo = document.querySelector('#infoModal');
        const modalOptions = { backdropClasses: 'bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40' }
        this.modalInfo = new Modal($modalElementInfo, modalOptions);

        // error modal
        const $modalElementError = document.querySelector('#errorModal');
        this.modalError = new Modal($modalElementError, modalOptions);
    }
